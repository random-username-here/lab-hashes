...
Погрешность взята в $3 \sigma$, таким образом вероятность попадания в данные границы $\approx 99.7\%$. **Это верно только при нормальном распределении измерений, но проверки данного факта в работе нет, потому что в таком случае нужно более "продвинутые" методы измерения**.
